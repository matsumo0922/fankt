...
公開 `Article.Block.Link` SHALL type と URL を持たない旧 serialized value を decode でき、その場合は type `unknown` と null URL を使用しなければならない。Trace: public serializable model への field 追加に対する非退行 invariant。
...
fixture test SHALL 保存済み actual response から匿名化した `default` / `html.card` / `fanbox.post` entry の field presence と型を保持し、完全な実値や credential を repository に含めず、公開 `Fanbox.getPostDetail` 経路を検証しなければならない。Trace: Issue #29 の3 fixture受け入れ条件と repository の golden fixture privacy contract。

#### Scenario: 3 type を production path で変換する

- **WHEN** actual-response-derived な3 type の匿名化 fixture を公開 `Fanbox.getPostDetail` から取得する test を実行する
