...
（agent 仮決め）`Text.StyleSpan(type, offset, length)` と `Text.LinkSpan(offset, length, url)` を公開 serializable data class とする。style type は文字列のまま保持し、`bold` 以外の未知値も拒否しない。offset / length は FANBOX が返した整数を加工せず保持する。
...
（agent 仮決め）transport `Block` に `styles: List<Style>? = null` と `links: List<Link>? = null` を追加し、欠落/null を public empty list へ正規化する。各 span field は actual response で確認した型を必須とし、型不一致は既存の known-block schema mismatch 契約へ流す。

### 4. p と header は text が存在すれば空文字でも Text を返す

（agent 仮決め）mapper は `block.text?.let` を使用し、`p` は `isHeader = false`、`header` は `isHeader = true` とする。空文字 p は `Text("")` として元の位置に残す。text が null の known p/header は現行どおり値を生成しないため、この change では新しい failure policy を導入しない。

### 5. span metadata は未信頼入力として保持し、適用は caller が検証する

（agent 仮決め）library は負値、範囲外、overlap、URL scheme を検査・補正せず受信値を保持する。KDoc は caller が text boundary と URL policy を検証してから描画・click 処理へ使う責務を記載する。これにより transport fidelity と UI security policy を分離する。
