...
The database SHALL migrate v1 Cookie rows to the canonical identity without losing a value for each identity, and SHALL treat migrated rows as secure-only because v1 did not persist the secure attribute. If v1 contains duplicate canonical identities, the row with the greatest SQLite `rowid` SHALL be retained deterministically because v1 stores no creation timestamp. The expiry column SHALL be indexed for request-time cleanup.
