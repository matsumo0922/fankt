...
- [ ] 2.2 Delete expired rows and filter reads with Ktor `Cookie.matches(requestUrl)`
...
- [ ] 3.2 Add an HTTP client test that proves the `HttpCookies -> PersistentCookieStorage` production call path does not emit cross-origin or expired Cookies
