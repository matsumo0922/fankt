...
- [ ] 1.1 Add `:fankt:fanbox-persistence-room` with Android/iOS, Room, detekt, and Maven publication configuration and a dependency on `:fankt:fanbox`.
- [ ] 1.2 Move the Room database, DAO, entity, migrations, schema files, persistent storage adapter, and platform path/build logic into the optional module.
- [ ] 1.3 Remove Room/sqlite/startup plugins, dependencies, sources, and initializer manifest metadata from `:fankt:fanbox`.
