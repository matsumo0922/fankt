...
- [x] 1.1 Decode article blocks as raw JsonObject values and add typed block `embedId` plus `embedMap` entries with `contentId` / `videoId`
...
- [x] 2.3 Normalize `videoId ?: contentId` and preserve unknown provider or missing content ID as referenced embed raw JSON

## 3. Fixture and production-path evidence

- [x] 3.1 Add a source-marked synthetic article embed fixture covering six known providers, videoId fallback, unknown provider, unknown block, and unresolved reference
