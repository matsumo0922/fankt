...
PR CI will run the declared-scope, Android ABI, and Android metadata gate, while the existing macOS job will run the iOS ABI gate. The release workflow will rerun the complete boundary gate before any publication step. A consumer fixture will resolve the property-versioned module metadata from an isolated repository rather than use a project dependency, reserve `me.matsumo.fankt` exclusively for that repository, verify the resolved artifact path, and compile public Flow, datetime, and serialization usages with no Ktor compile dependency. Its runtime graph still explicitly selects a compatible Ktor version.
