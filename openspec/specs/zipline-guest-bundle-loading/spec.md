# zipline-guest-bundle-loading Specification

## Purpose

Define how the FANBOX client loads a signed guest bundle, what it does when a signature fails or the
delivery target is unreachable, and when it stops using the guest — so that adding the delivery path
cannot make a request fail that would previously have succeeded.
